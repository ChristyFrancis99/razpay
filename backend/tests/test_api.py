...
def test_login_and_me():
    with TestClient(app) as client:
        ...
        assert resp.status_code == 200 and resp.json()["role"] == "ADMINISTRATOR"
...
def test_predict_valid_transaction():
    with TestClient(app) as client:
        ...
        resp = client.post("/api/transactions/predict", json=payload, headers=auth_headers(client))
        if resp.status_code == 503: pytest.skip("Model not trained yet")
        assert resp.status_code == 200
        body = resp.json(); assert 0 <= body["risk_score"] <= 100
...
def test_predict_missing_required_field():
    with TestClient(app) as client:
        assert client.post("/api/transactions/predict", json={"card4": "visa"}, headers=auth_headers(client)).status_code == 422


def test_get_unknown_transaction_returns_404():
    with TestClient(app) as client:
        assert client.get("/api/transactions/TXN-DOES-NOT-EXIST", headers=auth_headers(client)).status_code == 404


def test_get_unknown_merchant_returns_404():
    with TestClient(app) as client:
        assert client.get("/api/merchants/does-not-exist", headers=auth_headers(client)).status_code == 404


def test_copilot_with_no_matching_transaction():
    with TestClient(app) as client:
        resp = client.post("/api/copilot", json={"message": "why was this flagged?", "transaction_id": "TXN-UNKNOWN"}, headers=auth_headers(client))
        assert resp.status_code == 200
        assert resp.json()["engine"] in ("llm", "deterministic_template")
        assert "evidence" in resp.json()


def test_analytics_overview_ok_with_no_data():
    with TestClient(app) as client:
        resp = client.get("/api/analytics/overview", headers=auth_headers(client))
        assert resp.status_code == 200
