...
def health():
    ...
    model = get_model_bundle(); database_ok = False; db = SessionLocal()
    try: db.execute(text("SELECT 1")); database_ok = True
    finally: db.close()
    return {"status": "ok" if database_ok else "degraded", "database": "ok" if database_ok else "error", "model_loaded": model.is_ready, "model_name": model.metadata.get("model_name") if model.is_ready else None, "data_source": model.metadata.get("data_source") if model.is_ready else None}

@app.get("/api/health/ready", tags=["health"])
def readiness():
    from app.ml.predict import get_model_bundle
    model = get_model_bundle()
    if not model.is_ready: raise HTTPException(status_code=503, detail="Model is not ready. Train/load the model before serving predictions.")
    db = SessionLocal()
