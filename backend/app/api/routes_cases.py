...
def _validate_reference(db: Session, transaction_id: str | None, merchant_id: str | None) -> None:
    if transaction_id and not fraud_service.get_cached_transaction(db, transaction_id):
        raise HTTPException(status_code=404, detail=f"Transaction '{transaction_id}' not found.")
    if not transaction_id and not merchant_id:
        raise HTTPException(status_code=422, detail="transaction_id or merchant_id is required.")
...
def create_case(payload: CaseCreateRequest, db: Session = Depends(get_db)):
    _validate_reference(db, payload.transaction_id, payload.merchant_id)
    try:
...
def get_case_events(
    ...
    if not case_service.get_case(db, case_id):
        raise HTTPException(status_code=404, detail=f"Case '{case_id}' not found.")
