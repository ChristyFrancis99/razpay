...
def get_audit_logs(
    ...
    if new_decision and new_decision.upper() not in {"ALLOW", "REVIEW", "HOLD"}:
        raise HTTPException(status_code=422, detail="new_decision must be ALLOW, REVIEW, or HOLD.")
    return audit_service.list_audit_logs(
        ...
        actor=actor,
        new_decision=new_decision.upper() if new_decision else None,
    )
...
def create_audit_log(payload: AuditLogCreate, db: Session = Depends(get_db)):
    ...
    if payload.new_decision.upper() not in {"ALLOW", "REVIEW", "HOLD"}:
        raise HTTPException(status_code=422, detail="new_decision must be ALLOW, REVIEW, or HOLD.")
    if payload.risk_score != transaction.risk_score:
        raise HTTPException(status_code=409, detail="risk_score does not match the persisted transaction score.")
